...
## #15 — Statistiques globales du site (admin)

- [ ] **Accès admin réservé** : précondition — être connecté au back-office avec un compte NON-admin (rôle `entreprise`). Action — ouvrir `/estadisticas`. Résultat observable — redirection vers `/login?error=admin_required`, le dashboard global n'est jamais affiché. Répéter connecté en `admin` → le dashboard s'affiche.
- [ ] **Cohérence des totaux** : précondition — quelques fiches publiées avec des visites et des contacts WhatsApp enregistrés (via l'annuaire public). Action — comparer, pour chaque commerce, ses totaux sur sa page Estadísticas entrepreneur avec le dashboard admin. Résultat observable — les totaux « Visitas » et « Contactos por WhatsApp » du dashboard global égalent la somme des totaux par commerce.
- [ ] **Classement et période** : précondition — au moins deux commerces avec des nombres de contacts WhatsApp différents. Action — ouvrir `/estadisticas`, basculer le sélecteur jour/semaine/mois. Résultat observable — le classement liste les commerces du plus au moins contacté (ordre stable), et le graphe d'évolution se re-bucketise selon la période choisie sans casser les totaux ni le classement.

## #16 — Création de comptes entreprise seedés par l'admin (sans email)
